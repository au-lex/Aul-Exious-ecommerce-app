...
const  Accessdpage= () => {
  const { increaseItemCount, onItemAdd, items } = useOutletContext();
    ...
    const {Anames} = useParams();
    useEffect(()=> {
        const findProuct = () => {
            const newProduct = Accessdata.find((product) => product.Anames === Anames)
  ...
  return (
    ...
    <div className="w-[370px] h-[230px]  mr-4 ml-3  rounded-xl  sm:w-[500px] sm:h-[400px]  ">
            <span className="absoulte text-[2.2em] px-4">
                <Link to="/Accessories"> <i class="ri-arrow-left-line"></i></Link>
               </span>
          <img
          ...
          <div className="px-2 space-x-12 ">
            <p className="text-gray-700 lg:w-[50%]">{singleProduct.desc}</p>
          </div>
          <section className="flex justify-center mt-5 space-x-20 sm:py-0 sm:justify-start lg:py-0 lg:justify-start">
            ...
            h-[3rem] w-[10rem] bg-gray-800 text-white">
